...
function Etcd:set(key, val)
  self:keys_put(key, {value=val})
end

function Etcd:get(key)
  local res, status=self:keys_get(key)
  return res.node.value
end

function Etcd:ls(key)
  local ret=self:request{
    path    = '/v2/keys' .. key,
    method  = 'GET',
    params  = {}
  }
  local result={}
  for i=1, #ret.node.nodes do
    local item=ret.node.nodes[i]
